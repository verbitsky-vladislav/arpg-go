	...
	for y := 0; y <= H; y++ {
		for x := 0; x <= W; x++ {
			...
			case !k[0] && !k[1]: // северная кромка юбки — сплошная скала под траву
				...
				// Отдельного правила «опустить одиночный куб на ступени» здесь быть
				// не может: в эту ветку попадает только РОВНЫЙ участок гребня. На
				// ступени у соседней колонки скала стоит рядом выше, значит k[0]
				// или k[1] взведён и клетка уходит в default — наклонный угол
				// (1,0,1,1 / 0,1,1,1) из cliff_face. Он и рисует переход ступени.
	...
	for y := 0; y <= H; y++ {
		for x := 0; x <= W; x++ {
			...
			if !ok || len(ids) == 0 {
				...
				// Ступень внутри массива скалы бахромы НЕ получает: соседняя
				// колонка стоит рядом выше, стык не виден, а полупрозрачный тайл
				// травы читался бы светлой заплаткой посреди камня. Бахрома нужна
				// только там, где верхний срез выходит на открытую землю.
				openAbove := func(cx int) bool {
					return inRock(cx, y) && !inRock(cx, y-1) &&
						!inRock(cx-1, y-1) && !inRock(cx+1, y-1)
				}
				if !openAbove(x) && !openAbove(x-1) {
					continue
				}
