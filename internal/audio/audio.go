...
// Verify разбирает банк, не трогая звуковое устройство: манифесты, файлы
// эффектов и заголовки треков. Нужен сборщику ресурсов (tools/pack) — музыка
// там перекодируется, и проверять её должен тот же код, который потом играет.
func Verify(fsys fs.FS, root string) error {
