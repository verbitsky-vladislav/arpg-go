...
// MapRev — ревизия ГЕОМЕТРИИ карты внутри формата v1.
//
// Мир персонажа собирается один раз и живёт в сохранении вечно (scene.loadMap).
// Поля при этом добавляются: под-клетка физики стала вчетверо мельче, у объектов
// появилась точка опоры. Старая карта, прочитанная новым кодом, выглядит
// исправной, но врёт: спрайты рисуются по новым правилам, а сетка физики в ней
// испечена по старым — деревья стоят в стороне от своих барьеров.
//
// Поэтому у карты есть ревизия: не совпала — мир пересобирается из того же сида.
// Рельеф от этого не меняется (сид тот же), меняются объекты и физика.
const MapRev = 2

// MapV1 — карта целиком.
type MapV1 struct {
	Format   string     `json:"format"` // "map_format v1"
	Rev      int        `json:"rev"`    // ревизия геометрии, см. MapRev
...
type PropInst struct {
	...
	Base     [2]int `json:"base"`           // точка опоры внутри спрайта, пиксели
